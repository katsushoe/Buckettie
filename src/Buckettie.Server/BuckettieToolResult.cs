...
internal static class BuckettieToolResultMapper
{
    ...
    internal static string BitbucketCode(BitbucketError error, string operation) => error switch
    {
        ...
        BitbucketError.InvalidTagSource => "tag_source_invalid",
        BitbucketError.TagSourceNotFound => "tag_source_not_found",
    ...
    private static string NotFoundCode(string operation) => operation switch
    {
        ...
        "tag_create" => "tag_source_not_found",
    ...
    private static string EnglishMessage(string code) => code switch
    {
        ...
        "tag_source_invalid" => "Specify a tag source branch name or full 40-character commit SHA; no default source is used.",
        "tag_source_not_found" => "The tag source could not be found or is not visible in this repository. Check the source and repository access before retrying.",
    ...
    private static string JapaneseMessage(string code) => code switch
    {
        ...
        "tag_source_invalid" => "タグの作成元Branch名または完全40桁コミットSHAを指定してください。暗黙の作成元はありません。",
        "tag_source_not_found" => "このリポジトリでタグの作成元が見つからないか参照できません。作成元とアクセス権を確認してください。",
