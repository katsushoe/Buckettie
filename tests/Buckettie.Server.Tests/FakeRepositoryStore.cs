...
{
    private readonly Dictionary<string, RepositoryOptions> _repositories = new(StringComparer.OrdinalIgnoreCase);
    ...
    public Task<IReadOnlyDictionary<string, RepositoryOptions>> LoadAllAsync(CancellationToken cancellationToken) =>
        Task.FromResult<IReadOnlyDictionary<string, RepositoryOptions>>(
            new Dictionary<string, RepositoryOptions>(_repositories, StringComparer.OrdinalIgnoreCase));
