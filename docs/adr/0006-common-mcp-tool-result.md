# ADR 0006: Common MCP tool result

- Status: Accepted

## Context

Git and Bitbucket REST Gateways use operation-specific result and error enums. Exposing those internal shapes directly gives MCP clients inconsistent success fields, error locations, enum serialization, and not-found meanings. Tool output must remain stable while giving AI clients enough sanitized diagnostic context to explain Git failures without disclosing credentials or environment-specific sensitive values.

## Decision

Every Buckettie MCP Tool returns the common structured shape `{ ok, operation, repository, data, error }`. Successful results place operation-specific typed content in `data` and set `error` to null. Failures set `data` to null. The existing `code` and `message` fields remain stable, and Git failures additionally return `category`, `details`, `summary`, `suggestedAction`, `retryable`, and `correlationId`. `category` repeats the stable snake_case Git classification for explicit AI consumption. `details` contains bounded, sanitized Git diagnostic text when the failure originated from a Git command. The summary identifies the failed Git operation. A Server-boundary mapper converts every Git and Bitbucket REST error to a fixed snake_case code and fixed non-secret guidance. Context-dependent HTTP 404 results are mapped by operation to repository, branch, Pull Request, or Tag not-found codes.

Git command failures are classified, when recognizable, as `authentication_failed`, `network_error`, `permission_denied`, `conflict`, `non_fast_forward`, `working_tree_dirty`, or `remote_mismatch`. `git_failed` is reserved for unclassified failures. Every Git failure receives an opaque correlation ID which is returned to the client and written to the audit event. CLI repository commands call the same MCP tools and therefore expose the same structured contract.

`bitbucket_pr_merge` uses the provider-neutral mergeability statuses `mergeable`, `conflicting`, `blocked`, `calculating_retryable`, and `unknown_retryable`. A synchronous or completed asynchronous merge preserves the existing Bitbucket Pull Request fields and adds `mergeability_status: mergeable`. Provider task status `PENDING` is polled at most three times at 250-millisecond intervals. If it remains pending, the Tool returns `mergeability_calculating`, `calculating_retryable`, `retryable: true`, and `retry_after_seconds: 2`. An unrecognized or malformed transient result returns `mergeability_unknown` with `unknown_retryable` and the same retry guidance. Conflicts and repository-rule rejections return `conflicting` and `blocked` respectively and are not retryable without an external change.

## Alternatives

- Expose Gateway results unchanged: rejected because Git and REST output contracts differ and leak internal enum organization into MCP schemas.
- Return raw exception or upstream error text: rejected because it is unstable and may contain sensitive data.
- Return fixed guidance only: rejected because an AI client cannot explain an unclassified Git failure from the fixed `git_failed` message.
- Replace all internal result types with one generic type: rejected because Git and REST layers have different responsibilities and typed errors remain useful below the MCP boundary.

## Impact

All MCP output schemas share the same envelope while retaining typed success data. Existing clients can continue to evaluate `ok`, `error.code`, and `error.message`; new clients can use `error.category` and `error.details` to explain Git failures. Adding an internal Gateway error requires an explicit mapper decision or it safely falls back to `git_failed` or `bitbucket_api_error`.

## Security conditions

- Error messages, summaries, and suggested actions are fixed literals. Git `details` is the only field that may contain sanitized command output.
- Before Git stderr is placed in `details`, URL-like values, email addresses, absolute Windows and Unix paths, credential-like assignments, and control whitespace are redacted or normalized. The result is limited to 1024 characters. Raw stderr is never returned or written to audit logs.
- Bitbucket merge error bodies and asynchronous task links are used only for fixed classification. Provider messages and URLs are not returned to clients or written to audit logs.
- Only stable snake_case codes are exposed for automated handling.
- Policy failures remain normal structured Tool failures and do not bypass Gateway enforcement.

## Operational conditions

Transport- or schema-level MCP failures remain protocol errors because no Tool operation result exists. Cancellation that reaches a Gateway is returned as `cancelled`; server shutdown and transport cancellation may terminate the protocol request.

## Implementation, tests, and documentation

The Server project owns the common contract and mapper because it is the external MCP boundary. The Application project owns Git diagnostic sanitization so raw stderr cannot cross the Gateway boundary. Tests verify the common generic envelope, Git stderr classification, correlation IDs, fixed guidance, category and sanitized diagnostic details, and removal of sensitive values. This ADR is the authoritative contract for MCP Tool result formatting.
