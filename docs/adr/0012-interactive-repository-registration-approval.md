...
`Buckettie.Server` runs as a LocalSystem Windows Service in Session 0 and cannot show UI on the interactive desktop directly. On each registration request the service uses `WTSGetActiveConsoleSessionId` and `WTSQueryUserToken` to resolve the logged-on user's session token, then `CreateProcessWithTokenW` to launch a small dedicated executable, `Buckettie.ApprovalPrompt`, on `winsta0\default` in that user's session. The service and the prompt process exchange a fixed, length-prefixed JSON request/response over a per-request Named Pipe (a fresh GUID name, ACL-restricted to the resolved user's SID and LocalSystem); the pipe is created before the process is launched and torn down after the exchange. The Dialog fails closed: no response within the approval Timeout is treated as Denied.
