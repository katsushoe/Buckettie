...
## Explicit branch and tag source and partial status
...
`buckettie tag create <repository> <tag> <source> [--message X]` and MCP `bitbucket_tag_create` require an allowed source branch name or full 40-character commit SHA. A branch source is resolved through Bitbucket before creation; a commit source is verified through Bitbucket. The resolved SHA is sent as the tag target and checked against the response. Missing or invalid sources return `tag_source_invalid`; inaccessible or absent sources return `tag_source_not_found`. The response includes `source`, `source_kind`, `source_hash`, and `target_hash`. Omitting `--message` still omits the Bitbucket JSON `message` property.

`repo status` still returns local branch/HEAD/working-tree state when configured remote-tracking refs are missing. `remote_develop_head`/`remote_main_head` may be null; `ahead`/`behind` are null when comparison against develop is unavailable. `comparison_reference`, `comparison_unavailable_reason` and `missing_remote_references` explain the result. Missing refs can mean not fetched, not necessarily absent on Bitbucket. Other errors still fail. Provider capabilities report `contract_version: 3`, `branch_source_required: true`, `tag_source_required: true`, `repository_status_nullable: true`; Moyai callers must pass source unchanged and preserve nulls. See [ADR 0014](docs/adr/0014-explicit-branch-source-and-partial-status.md) and [ADR 0015](docs/adr/0015-explicit-tag-source.md).
